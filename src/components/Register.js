...
function Register() {
  ...
  const [email, setEmail] = useState('') //do przesyłania hasła z pola tekstowego do api i czyszczenia owego pola po kliknieciu przycisku
  const [error1, setError1] = useState('') //error dot pola loginu
  const [error2, setError2] = useState('') //error dot pola hasła
  const [error3, setError3] = useState('') //error dot pola hasła
  ...
  const loginHandler = () => {
    ...
    .catch(function (error) {
      ...
      setError2(error.response.data.email)
    ...
    <section class='section is-large'>
      ...
      <div class='column is-half is-offset-one-quarter'>
        ...
        <div class='field'>
          ...
          {error3 && <p class='has-text-centered has-text-danger'>{error3}</p>}
        </div>
